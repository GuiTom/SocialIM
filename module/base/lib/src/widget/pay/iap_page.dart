  ...
  static Future show(BuildContext context) {
    return Navigator.of(context).push(
      MaterialPageRoute<bool>(
        ...
        settings: const RouteSettings(name: '/IAPPage'),
      ),
    );
  }
...
class _IAPPageState extends State<IAPPage> {
  ...
  void initState() {
    ...
    _subscription =
      ...
      // handle error here.
      ToastUtil.showCenter(msg: error.toString());
    });
    initStoreInfo();
    super.initState();
  }
    ...
    productList.addAll(_products.map(
      (ProductDetails productDetails) {
        ...
        if (productDetails?.id == _kSilverSubscriptionId) {
          ...
          if (Session.yearlyVipExpireDate.isAfter(DateTime.now())) {
            enabled = false;
          }
        } else if (productDetails?.id == _kUpgradeId) {
          if (Session.userInfo.hasCrown) {
            enabled = false;
          }
        }
        return ListTile(
  ...
  Future<bool> _verifyPurchase(PurchaseDetails purchaseDetails) async {
    if (purchaseDetails.verificationData.source == 'app_store') {
      Resp? resp = await PayAPI.iosVerify(
